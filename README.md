...
- `styles.css` — standard browser CSS, including the existing responsive layout and exact brand colors: navy `#05325E`, red `#CA151E`, white `#FFFFFF`.
...
All booking, enrollment, consultation and quotation links open WhatsApp at **+96179123991**. The quotation message is general and does not specify AED.

The callback form keeps the existing name, enquiry audience and email fields. It validates the fields, then opens a WhatsApp draft containing those details. **The visitor must send the message in WhatsApp to complete the request.** The page does not claim the request has been received, and it provides an “Open WhatsApp draft” link if the new tab is blocked. Nothing is stored by the website. Without JavaScript, use the direct WhatsApp contact link.

The email contact remains `info@inkaz.me`.

## Content and maintenance

The navy is **#05325E**, sampled from the original supplied logo as its most frequent dark-blue pixel (RGB 5, 50, 94). It is used for the header, navy sections, footer and headings on white. Body text on white is charcoal **#30363D**. The red is **#CA151E**, sampled as the most frequent red pixel (RGB 202, 21, 30) in the original supplied logo. White text on this red has approximately 5.8:1 contrast. The logo artwork itself has not been recolored.
...
Edit wording directly in `index.html`. The styles are already compiled into ordinary CSS; no Tailwind installation is required. Keep `script.js` included for the mobile menu, course modals and callback form. The original framework project has not been overwritten. This ZIP contains the updated static website.
